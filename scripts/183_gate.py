#!/usr/bin/env python3
"""
183_gate.py -- IS THE HYPEREDGE INTERACTION TERM ACTIVE?

Patched copy of 164_faithful.py. Two additions, nothing else changed:

  --no-interaction   sets dyn = stat in score_hyperedge, zeroing the
                     HyperSAGNN interaction term (d_i - s_i)^2. The model
                     keeps its memory, drift and history aggregation; it
                     loses only the ability to score a node differently
                     depending on which set it is in.

  --date-min/--date-max   restrict to a date window, so the heterogeneous
                     (2020-2021) and clonal (2022+) regimes can be compared.

WHY
  180: from 2021-09 the population is one lineage group (n_components = 1,
       mean pairwise Jaccard 0.90).
  182: each node's context is near-identical across the sets containing it
       (ctx_jaccard 0.485 in 2020-21, 0.905 in the 2024-25 eval window).
       dyn is computed from the other members, so if contexts do not vary,
       dyn cannot vary, whatever weights are learned.
  174: additions are conditionally independent given the background
       (-0.003 nats) -- the same absence measured from the data side.

PREDICTION (falsifiable): ablating the interaction term costs accuracy in a
2021 window and costs nothing in a 2024 window. If it costs nothing in BOTH,
the term was never active and the regime story is wrong. If it costs in
BOTH, the input-side measurements in 182 are not capturing what the trained
model uses.
"""
"""
...
class DHyperNodeTPP(nn.Module):
    ...
    def __init__(self, V, d=64, heads=2, N=10, max_size=64,
        ...
        self.no_interaction = False   # set by --no-interaction
    ...
    def score_hyperedge(self, variants, t_now):
        ...
        if self.no_interaction:
            dyn = stat          # zero the hyperedge term
...
def run(a):
    ...
    if a.date_min or a.date_max:
        lo = a.date_min or '0000-00-00'
        hi = a.date_max or '9999-99-99'
        all_days = [u for u in all_days if lo <= days[u][:10] <= hi]
        print(f'date filter {lo}..{hi}: {len(all_days)} days')
    ...
    model.no_interaction = a.no_interaction
    if a.no_interaction:
        print('ABLATION: interaction term zeroed (dyn = stat)')
...
def main():
    ...
    p.add_argument("--no-interaction", action="store_true",
                   dest="no_interaction",
                   help="zero the HyperSAGNN interaction term")
    p.add_argument("--date-min", default=None, dest="date_min")
    p.add_argument("--date-max", default=None, dest="date_max")
