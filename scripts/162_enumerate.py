...
def enumerate_candidates(circ_mass, active_muts, radius=2,
                         top_circ=500, top_muts=500, exclude=None):
    """Exhaustive candidate set by local perturbation.

    159 showed <=2 mutations covers 90-97% of future mass at 1-3 months in
    the test period. So the ceiling is there -- enumeration reaches it, and
    the model only has to rank.

    No sampling, no generator, no coverage problem. The model's only job is
    to put mass on the right elements of this list.

    The node-level machinery -- GRU memory + neighbourhood attention -- feeds
    the scorer, not the generator, which is where it matters: the scorer asks
    whether each candidate's mutations are mutually coherent GIVEN their
    current representations. That is the interaction term the factorised
    mixture could not express, and it is the main machinery in this model.
    """
    exclude = exclude or set()
    ranked_circ = sorted(circ_mass, key=lambda kv: -kv[1])[:top_circ]
    ranked_muts = sorted(active_muts.items(), key=lambda kv: -kv[1])[:top_muts]
    top_m = [m for m, _ in ranked_muts]

    cands = set()
    for v, _ in ranked_circ:
        # radius 1: add one active mutation
        for m in top_m:
            if m not in v:
                cands.add(frozenset(v | {m}))
        # radius 1: remove one mutation (reversions)
        for m in v:
            c = v - {m}
            if c:
                cands.add(frozenset(c))
        if radius >= 2:
            # radius 2: add two mutations (top-100 only to keep it tractable)
            for i, m1 in enumerate(top_m[:100]):
                if m1 in v:
                    continue
                for m2 in top_m[i+1:100]:
                    if m2 not in v:
                        cands.add(frozenset(v | {m1, m2}))
...
def build_support(model, circ_mass, t, dt, rng, a, obs_mass=None,
                  train=True):
    """Enumerate candidates by local perturbation.

    The node machinery -- GRU memory + neighbourhood attention feeding v_i(t)
    -- is used in the scorer, not for generation. Generation is rule-based
    because 159 proved the ceiling is high (<=2 covers 90-97% at 1-3 months):
    the problem is ranking, not reaching.

    Observed variants that the enumeration missed are added during training so
    the fitness and scorer can learn to put mass on them. At evaluation they
    are excluded, so the reported number is honest.
    """
    circ = [v for v, _ in circ_mass]
    mass = torch.tensor([m for _, m in circ_mass], dtype=torch.float32)
    u = model.pop_state(circ, mass, float(t))

    # mutation activity: how much total mass carries each mutation right now
    ...
    cands = enumerate_candidates(
        circ_mass, act, radius=a.radius,
        top_circ=a.top_circ, top_muts=a.top_muts,
        exclude=set(circ))

    seen = set(circ) | set(cands)
    n_missed = 0
    if train and obs_mass:
        for v in obs_mass:
            if v not in seen:
                seen.add(v); cands.append(v); n_missed += 1
    pairs = [(v, None, None) for v in cands]
    return u, [], None, model.node_repr_cached(float(t)), pairs, n_missed
...
def transition_loss(model, circ_mass, obs_mass, t, dt, rng, a, train=True,
    ...
    mass = torch.tensor([m for _, m in circ_mass], dtype=torch.float32)
    u, _bg, _an, table, pairs, n_missed = build_support(
    ...
    meta = {"n_support": len(allv), "n_missed": n_missed,
            "n_target": len(obs_fit), "target_mass": run, "n_aux": 0, "aux": float("nan"),
...
def main():
    ...
    p.add_argument("--radius", type=int, default=2,
                   help="enumeration radius: 1=add/remove one, 2=add two")
    p.add_argument("--top-circ", type=int, default=500, dest="top_circ",
