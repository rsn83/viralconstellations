...
class SetEncoder(nn.Module):
    ...
    def embed(self, S_pca, S_bin):
        """Variant-level representations after horizontal mixing: (n, d)."""
        x = self.proj(S_pca)
    ...
    def forward(self, S_pca, S_bin, w):
        return self.pool(self.embed(S_pca, S_bin), w)
...
# ------------------------------------------------- SFCN-style components ---
class SFCNTemporal(nn.Module):
    """Linear mixing across months, in place of the LSTM.

    SFCNTSP mixes the time axis with a single trainable M x M matrix. With
    M=12 that is 144 parameters against roughly 25k for an LSTM with
    d_lstm=64 -- which matters at 23 training pairs. Future set-level
    representations come from a second l x M mixer rather than from
    autoregressive rollout, so there is no exposure bias either.
    """

    def __init__(self, M, l, d):
        super().__init__()
        # A single l x M matrix. Stacking a history mixer and a predictor would
        # multiply two linear maps, which collapses to one -- SFCNTSP make this
        # argument themselves and use a single layer.
        self.W_P = nn.Parameter(torch.zeros(l, M))
        with torch.no_grad():
            self.W_P[:, -1] = 1.0             # init = persistence
        self.M, self.l = M, l

    def forward(self, us):
        """us: (M, d) -> predicted future set-level representations (l, d)"""
        return self.W_P @ us


class ChannelMixer(nn.Module):
    """Mixing across the d embedding channels (SFCNTSP's third axis)."""
...
class VariantDecoder(nn.Module):
    """Derive the K mixture components from variant-level representations.

    The default decoder squeezes a whole month into one d-vector and
    regenerates K x r coefficients from it. Here K learned queries attend over
    the (broadcast) variant representations, so each component is anchored to
    actual variants in the population rather than to a single pooled vector.
    """

    def __init__(self, d, K, r):
        super().__init__()
        self.K, self.r = K, r
        self.queries = nn.Parameter(torch.randn(K, d) * 0.1)
        self.to_coeff = nn.Linear(d, r)
        self.to_logit = nn.Linear(d, 1)
        nn.init.zeros_(self.to_coeff.weight)
        nn.init.zeros_(self.to_coeff.bias)

    def forward(self, X, w, u_pred, P, logit_mu_t):
        # attention of each query over the variants present this month
        scores = (self.queries @ X.T) / (X.shape[-1] ** 0.5)      # (K, n)
        scores = scores + torch.log(w.clamp_min(1e-12)).unsqueeze(0)
        att = torch.softmax(scores, dim=1)
        z = att @ X                                                # (K, d)
        z = z + u_pred.unsqueeze(0)                                # temporal
        coeff = self.to_coeff(z)                                   # (K, r)
        pi = torch.softmax(self.to_logit(z).squeeze(-1), dim=0)
        return torch.sigmoid(coeff @ P + logit_mu_t.unsqueeze(0)), pi


class Seq2SeqSets(nn.Module):
    def __init__(self, V, r, d, K, heads, n_layers, d_lstm,
                 temporal="lstm", broadcast=False, channel_mix=False,
                 variant_decoder=False, M=12, l=6):
        super().__init__()
        self.d = d
        self.mode_temporal = temporal
        self.broadcast = broadcast
        self.variant_decoder = variant_decoder
        ...
        self.chan = ChannelMixer(d) if channel_mix else None
        self.dec_var  = VariantDecoder(d, K, r) if variant_decoder else None
        self.dec_mlp  = None if variant_decoder else MLPDecoder(d, K, r)
    ...
    def encode_month_full(self, S_bin, w):
        """Return (variant-level X, pooled u) for one month."""
        S_pca = (S_bin - self.global_mean) @ self.P.T
        x = self.set_enc.embed(S_pca, S_bin)          # (n, d)
        if self.chan is not None:
            x = self.chan(x)
        u = self.set_enc.pool(x, w)                   # (d,)
        return x, u

    def encode_month(self, S_bin, w):
        return self.encode_month_full(S_bin, w)[1]

    def _decode(self, u_pred, X_last, w_last, logit_mu_t):
        if self.dec_var is not None:
            Xb = X_last + u_pred.unsqueeze(0) if self.broadcast else X_last
            return self.dec_var(Xb, w_last, u_pred, self.P, logit_mu_t)
        return self.dec_mlp(u_pred, self.P, logit_mu_t)

    def forward_single(self, S_bin, w, logit_mu_t):
        X, u = self.encode_month_full(S_bin, w)
        return self._decode(u, X, w, logit_mu_t)

    def forward_seq2seq(self, window, logit_mu_t, l, teacher_us=None):
        Xs, us = [], []
        for d in window:
            x, u = self.encode_month_full(d["S_bin"], d["w"])
            Xs.append(x); us.append(u)
        us = torch.stack(us)

        if self.mode_temporal == "sfcn":
            us_pred = self.sfcn(us)                   # (l, d), no rollout
        else:
            h_t, ctx = self.temp_enc(us)
            us_pred  = self.dec_lstm(h_t, ctx, us[-1], l, teacher_us)

        X_last, w_last = Xs[-1], window[-1]["w"]
        return [self._decode(us_pred[s], X_last, w_last, logit_mu_t)
                for s in range(l)]
...
def main():
    ...
    # --- SFCNTSP-style components; defaults reproduce 143_seq2seq_v2 --------
    p.add_argument("--temporal", choices=["lstm", "sfcn"], default="lstm",
                   help="sfcn: M x M linear time mixer + l x M predictor")
    p.add_argument("--broadcast", action="store_true",
                   help="add predicted temporal context back to each variant")
    p.add_argument("--channel-mix", action="store_true", dest="channel_mix",
                   help="mix across the d embedding channels")
    p.add_argument("--variant-decoder", action="store_true",
                   dest="variant_decoder",
                   help="derive mixture components from variant "
                        "representations instead of the pooled vector")
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--out-tag", default="155", dest="out_tag")
    a = p.parse_args()

    torch.manual_seed(a.seed)
    np.random.seed(a.seed)
    if a.broadcast and not a.variant_decoder:
        print("note: --broadcast has no effect without --variant-decoder, "
              "since the default decoder consumes only the pooled vector")
    ...
    model = Seq2SeqSets(V=V, r=a.r, d=a.d, K=a.K,
                        ...
                        d_lstm=a.d_lstm,
                        temporal=a.temporal, broadcast=a.broadcast,
                        channel_mix=a.channel_mix,
                        variant_decoder=a.variant_decoder,
                        M=a.M, l=a.l)
