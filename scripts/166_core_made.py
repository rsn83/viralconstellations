...
class MADE(nn.Module):
    """Masked Autoregressive Density Estimator over K core positions.

    Models the full joint p(x_1,...,x_K | context) autoregressively.
    Each position is binary (mutation present/absent), conditioned on
    all previous positions AND the context vector (population state).

    This replaces the independent Bernoulli MLP_a for core positions,
    capturing the epistatic joint structure that makes assembly work.

    K=0 recovers 164 exactly (all residual Bernoulli, no MADE).
    """

    def __init__(self, K, d_ctx, d_hidden=128):
        super().__init__()
        self.K = K
        if K == 0:
            return
        # Input: context (d_ctx) + previous K positions (K binary)
        # Output: logit for current position
        self.net = nn.Sequential(
            nn.Linear(d_ctx + K, d_hidden), nn.Tanh(),
    ...
    def log_prob(self, x, ctx):
        """Log probability of binary vector x given context ctx.

        x   (B, K) binary, B variants
        ctx (B, d_ctx) or (d_ctx,) context vector
        ...
        prev = torch.zeros(B, K, device=x.device)
        for k in range(K):
            inp = torch.cat([ctx, prev], dim=-1)
            logit = self.net(inp).squeeze(-1)
            lp = lp + F.binary_cross_entropy_with_logits(
                logit, x[:, k], reduction='none') * (-1)
            prev = prev.clone(); prev[:, k] = x[:, k]
        return lp

    def sample(self, ctx, n=1):
        """Sample n binary vectors given context.

        Returns (n, K) binary tensor.
        """
        if self.K == 0:
...
class DHyperNodeTPP(nn.Module):
    ...
    def __init__(self, V, d=64, heads=2, N=10, max_size=64,
        ...
        # MADE for core positions (K_core=0 recovers 164 exactly)
        self.made = MADE(0, d)
        self._core_idx = None

        # Residual MLP: p(x_i | core, v_i) conditioned on core embedding
        self.MLP_res = nn.Sequential(
            nn.Linear(2*d, 2*d), nn.Tanh(), nn.Linear(2*d, d))
        self.res_bias = nn.Parameter(torch.zeros(V))

        # Population state projection for MADE context
        self.pop_proj = nn.Linear(d, d)
    ...
    def set_core(self, core_idx, d_ctx=None):
        """Set core mutation indices. Rebuild MADE only if K changes."""
        K_new = len(core_idx)
        d_ctx = d_ctx or self.d
        if K_new != self.made.K:
            # preserve weights if possible via state dict transfer
            old_state = self.made.state_dict() if self.made.K > 0 else None
            self.made = MADE(K_new, d_ctx, d_hidden=max(128, 2*d_ctx))
            self.made = self.made.to(self.mem.device)
            # can't transfer weights when K changes (different input size)
        self._core_idx = core_idx

    def pop_state(self, t_now, circ_mass):
        """Population state: mass-weighted mean of circulating variant reps."""
        if not circ_mass:
            return torch.zeros(self.d, device=self.mem.device)
        ...
        for v, w in circ_mass:
            ...
            if ms:
                ...
                weights.append(w)
        if not reps:
            return torch.zeros(self.d, device=dev)
        X = torch.stack(reps)
        W = torch.tensor(weights, device=dev)
        W = W / W.sum().clamp_min(1e-9)
        return self.pop_proj(torch.tanh((W.unsqueeze(-1) * X).sum(0)))
    ...
    def predict_adjacency(self, active_idx, t_now, ctx=None):
        """MLP_a: Bernoulli logits over V per active node.

        For residual positions: same as 164.
        For core positions: handled separately via MADE.
        """
        v = self.node_repr(t_now)[active_idx]
        q = self.MLP_a(v)
        table = self.node_repr(t_now)
        return q @ table.T + self.adj_bias.unsqueeze(0)

    def predict_residual(self, active_idx, t_now, core_sample):
        """p(x_i | core_sample, v_i) for non-core positions.

        core_sample (K_core,) binary -- the sampled core
        Returns (n_active, V) logits with core positions zeroed out.
        """
        v = self.node_repr(t_now)[active_idx]           # (n, d)
        if self._core_idx is not None and len(self._core_idx) > 0:
            dev = v.device
            core_emb = v.new_zeros(v.shape[0], self.d)
            core_t = torch.tensor(self._core_idx, device=dev, dtype=torch.long)
            core_reps = self.node_repr(t_now)[core_t]   # (K_core, d)
            # embed which core mutations are active
            w = core_sample.to(dev).float().unsqueeze(-1) if                 isinstance(core_sample, torch.Tensor) else                 torch.zeros(len(self._core_idx), 1, device=dev)
            core_emb = (core_reps * w).mean(0).unsqueeze(0).expand(
                v.shape[0], -1)
        else:
            core_emb = v.new_zeros(v.shape[0], self.d)
        feat = torch.cat([v, core_emb], dim=-1)
        q = self.MLP_res(feat)
        table = self.node_repr(t_now)
        logits = q @ table.T + self.res_bias.unsqueeze(0)
        # zero out core positions (handled by MADE)
        if self._core_idx is not None:
            dev = logits.device
            logits[:, torch.tensor(self._core_idx, device=dev,
                                   dtype=torch.long)] = -1e9
        return logits
...
def compute_losses(model, circ_mass, obs_mass, t, dt, a, train=True):
    ...
    if train and model._core_idx is not None and model.made.K > 0:
        ...
        if X_core.shape[0] > 0:
            ...
            loss_made = -(W_core * lp_core).sum() / W_core.sum().clamp_min(1e-9)
            if torch.isfinite(loss_made):
                loss = loss + a.w_a * loss_made
                info['ll_a_core'] = float(loss_made.detach())
...
def run(a):
    ...
    # Initialize MADE with the correct K before optimizer creation
    if a.k_core > 0:
        # use first training day's top-K mutations to initialize
        first_cm = sorted(mass_map.get(train_days[0], {}).items(),
                          key=lambda kv: -kv[1])[:a.pop_support]
        if first_cm:
            act0 = {}
            for v2, w2 in first_cm:
                for m2 in v2:
                    if m2 < V: act0[m2] = act0.get(m2, 0.0) + w2
            core0 = sorted(act0, key=lambda m: -act0[m])[:a.k_core]
            model.set_core(core0, d_ctx=a.d)

    opt = torch.optim.Adam(model.parameters(), lr=a.lr)

    def update_core(t, cm):
        """Identify top-K_core mutations by mass and update MADE."""
        if a.k_core == 0:
            return
        act = {}
        for v, w in cm:
            for m in v:
                if m < V:
                    act[m] = act.get(m, 0.0) + w
        core = sorted(act, key=lambda m: -act[m])[:a.k_core]
        model.set_core(core, d_ctx=a.d)
...
def main():
    ...
    p.add_argument("--k-core", type=int, default=50, dest="k_core",
                   help="core positions for MADE; 0 recovers 164 exactly")
