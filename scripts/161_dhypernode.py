...
class AdjacencyGenerator(nn.Module):
    """DHyperNodeTPP's candidate generator (Gracious, Gupta, Dukkipati, AAAI-25).

    Their stage 2, undirected. For each node observing an event:

        a_i ~ Bernoulli(sigma(MLP_a(v_i)))    which nodes it will co-occur with
        k_i ~ Bernoulli(sigma(MLP_s(v_i)))    the size of the hyperedge

    Trained by LL_a and LL_k -- dense binary supervision on the observed
    adjacency vector and size, one label per node per event. That is a far
    denser gradient than routing through a forecast softmax, which is why
    their generator trains where a sampled one stalls.

    Candidates are assembled by taking a node's top-ranked neighbours up to its
    predicted size. This step is lossy and they name it as their bottleneck:
    {A,B}+{C,D} and {A,C}+{B,D} produce similar adjacency, so the assembly
    cannot tell them apart. It is also unbounded -- it can build a set from
    nodes that have never co-occurred, which anchored copying cannot.
    """

    def __init__(self, d, V, max_size=64):
        super().__init__()
        self.V, self.max_size = V, max_size
        self.adj = nn.Sequential(nn.Linear(d, 2 * d), nn.Tanh(),
                                 nn.Linear(2 * d, d))
        self.size = nn.Sequential(nn.Linear(d, 2 * d), nn.Tanh(),
                                  nn.Linear(2 * d, max_size))
        self.node_event = nn.Sequential(nn.Linear(d, d), nn.Tanh(),
                                        nn.Linear(d, 1))
        self.bias = nn.Parameter(torch.zeros(V))

    def adj_logits(self, vn, table):
        """vn (n, d) node representations -> (n, V) adjacency logits."""
        return self.adj(vn) @ table.T + self.bias.unsqueeze(0)

    def size_logits(self, vn):
        return self.size(vn)

    def event_logits(self, vn):
        return self.node_event(vn).squeeze(-1)

    def assemble(self, nodes, vn, table, n_cand, rng, exclude=None):
        """Build candidates from top-ranked neighbours, as in their stage 2."""
        exclude = exclude or set()
        out = []
        with torch.no_grad():
            A = torch.sigmoid(self.adj_logits(vn, table))          # (n, V)
            S = torch.softmax(self.size_logits(vn), dim=-1)        # (n, K)
            order = torch.argsort(A, dim=-1, descending=True)
            for _ in range(n_cand):
                i = rng.randrange(len(nodes))
                k = int(torch.multinomial(S[i], 1)) + 1
                members = {int(nodes[i])}
                members.update(int(x) for x in order[i, :max(k - 1, 0)])
                v = frozenset(members)
...
class TimeMixer(nn.Module):
    ...
    From SFCNTSP, and the piece that was stable at long horizons in 155 where
    an autoregressive rollout was not. 160 recomputed the population state
    fresh each day, so nothing looked across months at population level; the
    only history was per-mutation memory, which is a blurred summary.

    Identity-initialised, so at the start this is exactly the current state.
    """
    ...
    def __init__(self, d, M=8):
        ...
        with torch.no_grad():
            self.W[-1] = 1.0                  # start = most recent state only
    ...
    def forward(self, u_now):
        ...
        hist = (w.unsqueeze(-1) * self.buf).sum(0)
        return u_now + hist            # residual: never worse than the snapshot
...
class VariantTPP(nn.Module):
    ...
    def __init__(self, V, d=64, heads=2, n_recent=10, posres=None,
        ...
        self.gen = AdjacencyGenerator(d, V)
    ...
    def pop_state(self, variants, mass, t_now, mix=True):
        u = self.pop_enc(self.variant_repr(variants, t_now), mass)
        return self.mixer(u) if mix else u
...
def run(a):
    ...
    rows = []
    for T in origins:
        model.reset_state(); model.mixer.reset()
        for t in all_days:
            if t > T:
                break
            model.flush_pending(float(t))
...
def build_support(model, circ_mass, t, dt, rng, a, obs_mass=None,
                  train=True):
    """Support: circulating + candidates assembled from predicted adjacency.

    Candidates come from their stage 2 -- pick an active mutation, take its
    top-ranked predicted neighbours up to its predicted size. Unlike anchored
    copying this can build sets whose members have never co-occurred, so reach
    is not capped by any radius.
    """
    ...
    active = sorted({m for v in circ for m in v})
    if a.n_active and len(active) > a.n_active:
        cnt = defaultdict(float)
        for (v, w) in circ_mass:
            for m in v:
                cnt[m] += w
        active = [m for m, _ in sorted(cnt.items(),
                                       key=lambda kv: -kv[1])[:a.n_active]]
    an = torch.tensor(active, dtype=torch.long)
    vn = table[an]

    seen = set(circ)
    pairs = []
    for v in model.gen.assemble(active, vn, table, a.n_cand, rng, seen):
        if v not in seen:
    ...
    if train and obs_mass:
        for v in obs_mass:
            if v not in seen:
                seen.add(v); pairs.append((v, None, None)); n_missed += 1
    return u, active, an, vn, table, pairs, n_missed
...
def transition_loss(model, circ_mass, obs_mass, t, dt, rng, a, train=True,
                    return_parts=False):
    """Forecast loss plus their LL_a / LL_k / node-event supervision.

    LL_a and LL_k are their generator's training signal: for every active
    mutation, a binary label per vocabulary entry saying whether it co-occurs
    with that mutation in the observed population at t+dt, and a label for the
    observed hyperedge size. Thousands of dense labels per step, against the
    56-or-so supervision pairs a sampled generator gets -- this is the reason
    their generator trains.
    """
    circ = [v for v, _ in circ_mass]
    ...
    mass = torch.tensor([m for _, m in circ_mass], dtype=torch.float32)
    u, active, an, vn, table, pairs, n_missed = build_support(
    ...
    # ---- LL_a and LL_k: their dense generator supervision ---------------
    aux = None
    n_aux = 0
    if train and a.aux_weight > 0 and obs_fit:
        pos = torch.zeros(len(active), model.V)
        szl = torch.zeros(len(active), model.gen.max_size)
        row = {m: i for i, m in enumerate(active)}
        tot_o = sum(obs_fit.values()) or 1.0
        wrow = torch.zeros(len(active))
        for v, wv in obs_fit.items():
            ms = [m for m in v if m in row]
            if not ms:
                continue
            k = min(len(v), model.gen.max_size) - 1
            idx = torch.tensor([row[m] for m in ms], dtype=torch.long)
            cols = torch.tensor([m for m in v if m < model.V],
                                dtype=torch.long)
            pos[idx.unsqueeze(1), cols.unsqueeze(0)] = 1.0
            szl[idx, k] = 1.0
            wrow[idx] += wv / tot_o
        live = wrow > 0
        n_aux = int(live.sum())
        if n_aux:
            la = F.binary_cross_entropy_with_logits(
                model.gen.adj_logits(vn[live], table), pos[live],
                reduction="none").mean(1)
            lk = F.binary_cross_entropy_with_logits(
                model.gen.size_logits(vn[live]), szl[live],
                reduction="none").mean(1)
            ev = F.binary_cross_entropy_with_logits(
                model.gen.event_logits(vn), (wrow > 0).float())
            w = wrow[live]
            aux = ((w * (la + lk)).sum() / w.sum().clamp_min(1e-9)) + ev
            if torch.isfinite(aux):
                loss = loss + a.aux_weight * aux

    meta = {"n_support": len(allv), "n_missed": n_missed,
            "n_target": len(obs_fit), "target_mass": run, "n_aux": n_aux,
            "aux": float(aux.detach()) if aux is not None else float("nan"),
            "budget": float(torch.sigmoid(model.budget_logit(dt)).detach()),
            "mix_anchored": float("nan")}
    if return_parts:
...
def main():
    ...
    p.add_argument("--n-cand", type=int, default=1000, dest="n_cand",
                   help="candidates assembled from predicted adjacency")
    p.add_argument("--n-active", type=int, default=500, dest="n_active",
                   help="active mutations considered, heaviest first")
