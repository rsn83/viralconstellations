...
def main():
    ...
    ap.add_argument("--k", type=int, default=10,
                    help="subspace dimension for the stability test")
    ...
    for m in months:
        ...
        muts = sorted({i for S in sets for i in S})
        if len(sets) < 5 or len(muts) < 5:
            continue
        H = incidence(sets, muts)
        st = spectrum_stats(H, rng)
        if st is None:
            continue
        U, _, _ = np.linalg.svd(H, full_matrices=False)
        subs[m] = (muts, U[:, :a.k])
        st.update({"month": m, "n_sets": len(sets), "n_muts": len(muts)})
        rows.append(st)
    ...
    print(f"\n  TEST A -- does low-rank structure exist?")
    print(f"  {'month':9s} {'sets':>5s} {'muts':>5s} {'top5':>7s} "
          f"{'top10':>7s} {'top30':>7s} {'effrank':>8s} {'null':>7s}")
    print("  " + "-" * 62)
    for r in rows:
        print(f"  {r['month']:9s} {r['n_sets']:5d} {r['n_muts']:5d} "
              f"{r['frac_top5']:7.3f} {r['frac_top10']:7.3f} "
              f"{r['frac_top30']:7.3f} {r['eff_rank']:8.1f} "
              f"{r['eff_rank_null']:7.1f}")
