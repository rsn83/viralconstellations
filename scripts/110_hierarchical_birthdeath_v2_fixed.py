...
def m_step(model, ks, Sd, N, tv, drift, inner, lr, rw,
    ...
    summed over months, and the curvature is  N[t,i] * theta * (1 - theta).
    Because a child's profile is the sum of deviations along its path to the
    root, changing a parent moves every descendant -- so a node's gradient AND
    curvature with respect to its OWN deviation are its own plus all of its
    descendants'. Under hier_drift the slopes are path-summed too, so their
    curvature accumulates the same way; otherwise the slope is local.

    The Gaussian prior contributes -delta / sigma^2 to the gradient and
    1 / sigma^2 to the curvature. A Beta(a,a) prior on theta adds
    (a-1)(1 - 2*theta) to the gradient and 2(a-1)*theta*(1-theta) to the
    curvature.

    WHY NEWTON AND NOT A FIXED STEP
        The earlier version used a single step size lr / N.sum() for every
        coordinate. The logistic curvature spans four orders of magnitude
        across positions in a sparse mutation matrix, so a step tuned for
        theta ~ 0.5 is roughly 5000x too small at theta ~ 1e-4. Rare
        positions -- most of them, and the ones novel sets depend on --
        barely moved in `inner` iterations. Worse, the slope step was scaled
        by the same 1/tot, which left gamma near its initial value: the
        'drift' rungs were effectively driftless.

    WHY delta AND gamma ARE SOLVED TOGETHER
        They are coupled through theta = sig(beta + gamma*t). Stepping each
        as if the other were fixed and then moving both overshoots, badly
        enough to diverge. The 2x2 block solve is exact and costs nothing.

    NOTE ON lr
        lr is DAMPING on a Newton step, not a learning rate. 1.0 is a full
        step; the old default of 2.0 will overshoot.

    NOTE ON weight_profile
        The earlier version applied the recency weights rw to the slope
        gradient but not the profile gradient, so beta and gamma ascended
        two different objectives and monotonicity was undefined. Both now
        use the same weights.
    """
    V = model.V
    K = len(ks)
    kidx = {int(k): i for i, k in enumerate(ks)}
    order = sorted(range(K), key=lambda i: -model.depth(int(ks[i])))
    inv_s2 = 1.0 / model.sigma ** 2
    # Without hier_drift the slope has no prior of its own, and with recency
    # weighting it sees only a few effective months -- near-singular curvature,
    # and an unregularised Newton step sends gamma to infinity. Under
    # hier_drift sigma already supplies the shrinkage.
    inv_t2 = 0.0 if model.hier_drift else 1.0 / max(gamma_tau, 1e-6) ** 2
    a_pr = model.beta_prior - 1.0
    wp = rw if weight_profile else np.ones(len(Sd))

    def objective():
        val = 0.0
        for t in range(len(Sd)):
            th, _ = model.theta(tv[t], drift)
            th = np.clip(th, 1e-9, 1 - 1e-9)
            val += wp[t] * float((Sd[t] * np.log(th)).sum()
                                 + ((N[t][:, None] - Sd[t])
                                    * np.log(1 - th)).sum())
            if a_pr != 0.0:
                val += wp[t] * a_pr * float((np.log(th) + np.log(1 - th)).sum())
        for k in ks:
            d = model.delta[int(k)]
            val -= 0.5 * inv_s2 * float((d * d).sum())
            g_ = model.gamma[int(k)]
            if model.hier_drift:
                val -= 0.5 * inv_s2 * float((g_ * g_).sum())
            elif drift and inv_t2 > 0:
                val -= 0.5 * inv_t2 * float((g_ * g_).sum())
        return val

    obj0 = objective(); prev_obj = obj0

    for _ in range(inner):
        gb = np.zeros((K, V)); hb = np.zeros((K, V))
        gg = np.zeros((K, V)); hg = np.zeros((K, V)); hc = np.zeros((K, V))
        for t in range(len(Sd)):
            th, _ = model.theta(tv[t], drift)
            g = Sd[t] - N[t][:, None] * th
            h = N[t][:, None] * th * (1.0 - th)
            if a_pr != 0.0:
                g = g + a_pr * (1.0 - 2.0 * th)
                h = h + 2.0 * a_pr * th * (1.0 - th)
            gb += wp[t] * g; hb += wp[t] * h
            if drift:
                gg += wp[t] * g * tv[t]
                hg += wp[t] * h * tv[t] ** 2
        ...
        # delta always accumulates up the tree
        acc_g = gb.copy(); acc_h = hb.copy()
        for i in order:
            p = int(model.parent[int(ks[i])])
            if p in kidx:
                acc_g[kidx[p]] += acc_g[i]; acc_h[kidx[p]] += acc_h[i]
        if model.hier_drift and drift:
            # slopes are path-summed too, so their curvature and their cross
            # term with delta accumulate identically
            for i in order:
                p = int(model.parent[int(ks[i])])
                if p in kidx:
                    gg[kidx[p]] += gg[i]; hg[kidx[p]] += hg[i]
                    hc[kidx[p]] += hc[i]

        step_d = np.zeros((K, V)); step_g = np.zeros((K, V))
        for i, k in enumerate(ks):
            k = int(k)
            gd = acc_g[i] - model.delta[k] * inv_s2
            A = acc_h[i] + inv_s2
            if drift:
                pg = inv_s2 if model.hier_drift else inv_t2
                gm = gg[i] - model.gamma[k] * pg
                D = hg[i] + pg
                C = hc[i]
                det = np.maximum(A * D - C * C, EPS)
                step_d[i] = (D * gd - C * gm) / det
                step_g[i] = (A * gm - C * gd) / det
            else:
                step_d[i] = gd / np.maximum(A, EPS)

        # Trust region. A component whose mass sits in a single month has a
        # nearly collinear (delta, gamma) block, and a component with almost
        # no mass has almost no curvature; either way the Newton step can be
        # astronomically large and twelve halvings will not bring it back.
        # No logit needs to move more than a few units in one step.
        np.clip(step_d, -max_step, max_step, out=step_d)
        np.clip(step_g, -max_step, max_step, out=step_g)

        # Backtracking. The tree accumulation approximates the full Hessian
        # (it ignores curvature shared between siblings), so guard the step
        # rather than assume it ascends.
        d_save = {int(k): model.delta[int(k)].copy() for k in ks}
        g_save = {int(k): model.gamma[int(k)].copy() for k in ks}
        step = lr; cur = None
        for _try in range(12):
            ...
            for k in ks:
                k = int(k)
                model.delta[k] = d_save[k]; model.gamma[k] = g_save[k]
            cur = prev_obj
            if verbose:
                print("      m_step: no ascending step, stopping", flush=True)
            break

        if abs(cur - prev_obj) < tol * max(1.0, abs(prev_obj)):
            prev_obj = cur; break
        prev_obj = cur

    if verbose:
        print(f"      m_step obj {obj0:.6e} -> {prev_obj:.6e} "
              f"({prev_obj - obj0:+.3e})", flush=True)
    return model
...
def try_remerge(Xs, ws, model, Pi, tv, drift, p_birth, names, verbose,
                inner=8, lr=1.0, rw=None, refit=3, n_pairs=3, min_side=50.0,
                weight_profile=True, gamma_tau=1.0,
                diag=None):
    ...
    # Refit the INCUMBENT on the same budget the challenger gets below.
    # Scoring a thoroughly refit proposal against a model evaluated where it
    # happens to stand makes lp1 - lp0 report the incumbent's un-refit
    # deviations rather than anything about the birth: with a converging
    # M-step the refit shrinks every delta, so dPrior swamps dLL by orders of
    # magnitude and births are accepted on an artefact. (Harmless only while
    # neither side converged.)
    _snap0 = _snapshot(model)
    Pi0 = Pi
    for _ in range(refit):
        _S, _Sd, _nm, _N, _, _ks = e_step(Xs, ws, model, Pi0, tv, drift)
        model = m_step(model, _ks, _Sd, _N, tv, drift, inner, lr, rw,
                       weight_profile=weight_profile, gamma_tau=gamma_tau)
        Pi0 = _N / np.maximum(_N.sum(1, keepdims=True), EPS)
    *_, ll0, _ = e_step(Xs, ws, model, Pi0, tv, drift)
    lp0 = log_prior(model, p_birth)
    _restore(model, _snap0)
...
def try_birth(Xs, ws, model, Pi, tv, drift, p_birth, names, verbose,
              inner=8, lr=1.0, rw=None, refit=5, n_cand=3, min_side=50.0,
              weight_profile=True, gamma_tau=1.0,
...
def fit(Xs, ws, V, max_K, seed=0, iters=200, inner=8, lr=1.0, sigma=1.5,
...
def fit_flat(Xs, ws, V, K, seed=0, drift=False, split_merge=False,
             iters=200, inner=8, lr=1.0, half_life=1.0, prior=.5,
             names=None, verbose=False, init_beta=None, return_gamma=False,
             return_pi=False, weight_profile=True, gamma_tau=1.0):
    ...
    for it in range(iters):
        ...
        if not drift:
            ...
            # Joint 2x2 Newton on (beta, gamma) per coordinate. They are
            # coupled through theta = sig(beta + gamma*t), so separate steps
            # each assume the other is fixed and then both move -- that
            # overshoots. gamma carries a ridge because recency weighting
            # leaves the slope with few effective months.
            wp = rw if weight_profile else np.ones(T)
            inv_t2 = 1.0 / max(gamma_tau, 1e-6) ** 2
            for _ in range(inner):
                gb = np.zeros((K, V)); A = np.zeros((K, V))
                gg = np.zeros((K, V)); D = np.zeros((K, V)); C = np.zeros((K, V))
                for t in range(T):
                    th_t = th_at(t)
                    g = Sd[t] - N[t][:, None] * th_t
                    h = N[t][:, None] * th_t * (1.0 - th_t)
                    gb += wp[t] * g;         A += wp[t] * h
                    gg += wp[t] * g * tv[t]; D += wp[t] * h * tv[t] ** 2
                    C  += wp[t] * h * tv[t]
                gg = gg - gamma * inv_t2; D = D + inv_t2
                det = np.maximum(A * D - C * C, EPS)
                beta  = beta  + lr * (D * gb - C * gg) / det
                gamma = gamma + lr * (A * gg - C * gb) / det
        Pi = N / np.maximum(N.sum(1, keepdims=True), EPS)
                ...
                if k >= 0:
                    ...
                    for t, (X, w) in enumerate(zip(Xs, ws)):
                        ...
                        if B_.any():
                            ...
                            mb[t] = w[B_].sum() / w.sum()
                    if da > 50 and db > 50:
                        # Only rows k and d may change. The previous version
                        # rebuilt the WHOLE beta matrix from th_at(T-1), which
                        # silently set beta := beta + gamma*tv[-1] for every
                        # component while leaving gamma alone -- the drift was
                        # then applied twice. Invisible while gamma was stuck
                        # near zero; real corruption once slopes are fitted.
                        p_d = np.clip((na + .5) / (da + 1.), 1e-4, 1 - 1e-4)
                        p_k = np.clip((nb + .5) / (db + 1.), 1e-4, 1 - 1e-4)
                        b_bak = beta.copy(); g_bak = gamma.copy()
                        P_bak = Pi.copy()

                        def _em1(b, g, P):
                            Sd3 = []; N3 = np.zeros((T, K))
                            for t3, (X3, w3) in enumerate(zip(Xs, ws)):
                                th3 = np.clip(sig(b + g * tv[t3]) if drift
                                              else sig(b), 1e-4, 1 - 1e-4)
                                lp3 = (loglik_matrix(X3, th3)
                                       + np.log(P[t3] + EPS)[None, :])
                                mx3 = lp3.max(1, keepdims=True)
                                P3 = np.exp(lp3 - mx3)
                                Rw3 = (P3 / P3.sum(1, keepdims=True)) * w3[:, None]
                                Sd3.append(Rw3.T @ X3); N3[t3] = Rw3.sum(0)
                            if not drift:
                                num3 = sum(Sd3); den3 = N3.sum(0)
                                th3 = np.clip((num3 + prior)
                                              / (den3[:, None] + 2 * prior),
                                              1e-4, 1 - 1e-4)
                                b = np.log(th3 / (1 - th3))
                            else:
                                wp3 = rw if weight_profile else np.ones(T)
                                it3 = 1.0 / max(gamma_tau, 1e-6) ** 2
                                gb3 = np.zeros((K, V)); A3 = np.zeros((K, V))
                                gg3 = np.zeros((K, V)); D3 = np.zeros((K, V))
                                C3 = np.zeros((K, V))
                                for t3 in range(T):
                                    th3 = np.clip(sig(b + g * tv[t3]),
                                                  1e-4, 1 - 1e-4)
                                    r3 = Sd3[t3] - N3[t3][:, None] * th3
                                    h3 = N3[t3][:, None] * th3 * (1 - th3)
                                    gb3 += wp3[t3] * r3; A3 += wp3[t3] * h3
                                    gg3 += wp3[t3] * r3 * tv[t3]
                                    D3 += wp3[t3] * h3 * tv[t3] ** 2
                                    C3 += wp3[t3] * h3 * tv[t3]
                                gg3 -= g * it3; D3 += it3
                                det3 = np.maximum(A3 * D3 - C3 * C3, EPS)
                                b = b + (D3 * gb3 - C3 * gg3) / det3
                                g = g + (A3 * gg3 - C3 * gb3) / det3
                            P = N3 / np.maximum(N3.sum(1, keepdims=True), EPS)
                            return b, g, P
                        ...
                        def _obj(b, g, P):
                            ...
                            for t3, (X3, w3) in enumerate(zip(Xs, ws)):
                                ...
                                lp3 = (loglik_matrix(X3, th3)
                                       + np.log(P[t3] + EPS)[None, :])
                                mx3 = lp3.max(1, keepdims=True)
                                v += float((w3 * (np.log(np.exp(lp3 - mx3).sum(1))
                                                  + mx3.ravel())).sum())
                            return v

                        # give the INCUMBENT the same EM steps as the
                        # challenger, and charge for the extra parameters --
                        # otherwise any split improves the training objective
                        # and is accepted
                        _b0, _g0, _P0 = b_bak.copy(), g_bak.copy(), P_bak.copy()
                        for _ in range(5):
                            _b0, _g0, _P0 = _em1(_b0, _g0, _P0)
                        before = _obj(_b0, _g0, _P0)
                        n_obs = float(sum(w_.sum() for w_ in ws))
                        pen = 0.5 * (2 * V if drift else V) * np.log(max(n_obs, 2.))

                        beta[d] = np.log(p_d / (1 - p_d))
                        beta[k] = np.log(p_k / (1 - p_k))
                        # both halves are estimated pooled over months, so
                        # their slopes restart from zero and are relearned
                        gamma[d] = 0.; gamma[k] = 0.
                        Pi[:, d] = ma; Pi[:, k] = mb
                        Pi = Pi / np.maximum(Pi.sum(1, keepdims=True), EPS)
                        for _ in range(5):
                            beta, gamma, Pi = _em1(beta, gamma, Pi)

                        if _obj(beta, gamma, Pi) - pen <= before:
                            beta, gamma, Pi = b_bak, g_bak, P_bak
                            if verbose:
                                print(f"      split blk{k} rejected (no gain)",
                                      flush=True)
                        else:
                            splits.append((k, d, n_mut, sc))
                            if verbose:
                                nm = (names or {}).get(n_mut, str(n_mut))
                                print(f"      split blk{k} on {nm} -> blk{d}",
                                      flush=True)
...
def main():
    ...
    ap.add_argument("--inner", type=int, default=8,
                    help="M-step Newton iterations per EM step. Raise it to "
                         ...
                         "the fit was not converged.")
    ap.add_argument("--gamma-tau", type=float, default=1.0,
                    help="ridge on the drift slopes when --hier-drift is OFF. "
                         "Recency weighting leaves the slope with few "
                         "effective months, so an unregularised Newton fit "
                         "diverges. Ignored under --hier-drift, where sigma "
                         "supplies the shrinkage. Sweep this.")
    ap.add_argument("--unweighted-profile", action="store_true",
                    help="fit profiles on the unweighted likelihood while "
                         "slopes use recency weights. NOT a single objective "
                         "-- reproduces the old behaviour only.")
    ...
    for sd in range(args.seeds):
        ...
        th, pi, used, _, _, warm = fit_flat(Xs, ws, V, args.max_K, seed=sd,
                     ...
                     weight_profile=not args.unweighted_profile,
                     gamma_tau=args.gamma_tau, inner=args.inner)
